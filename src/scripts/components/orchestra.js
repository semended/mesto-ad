...
export function conductAIOrchestra() {
  // Пасхалка не имеет права уронить «Место». Любая ошибка внутри
  // глушится здесь и в худшем случае приложение просто без оваций.
  try {
    performConcert();
  } catch (e) {
    console.warn("AI-оркестр сфальшивил, но «Место» играет дальше:", e);
  }
}
...
function playWedding() {
  try {
    ...
    frame.tabIndex = -1; // не ловит фокус, не уводит скролл
    frame.setAttribute("aria-hidden", "true");
    frame.src =
      `https://www.youtube.com/embed/${WEDDING_VIDEO_ID}` +
      `?autoplay=1&controls=0&modestbranding=1&rel=0&start=${WEDDING_START_SEC}`;
    // Нулевой размер и вне потока — на разметку «Места» не влияет
    // вообще никак. display:none не ставим: глушит звук.
    frame.style.cssText =
      "position:fixed;width:0;height:0;left:-9999px;bottom:0;" +
      "border:0;opacity:0;pointer-events:none;clip:rect(0 0 0 0);";
    document.body.appendChild(frame);
  } catch (e) {
    console.warn("Плеер не завёлся, но «Место» работает:", e);
    return;
  }
